...
def skill_ner(doc):
    ...
    blocks = [ent for ent in doc.ents if ent.label_ == 'Skills']
...
def degree_ner(doc):
    ...
    # Degree pattern
    degree_pattern = set()
    for _ , degrees in cs.DEGREE.items():
        degree_pattern.update({degree.lower() for degree in degrees})
    
    original_ents = list(doc.ents)
    blocks = [ent for ent in doc.ents if ent.label_ == 'Degree']
    ...
    for ent in mwt_ents:
        ...
        words = text.split()
        for word in words:
            if word.lower() in degree_pattern:
